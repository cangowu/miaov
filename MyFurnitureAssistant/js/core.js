...
(function (__host) {
    ...
    //var dilateGray = function (__src, __size, __borderType, __dst) {
    //    __src || error(arguments.callee, IS_UNDEFINED_OR_NULL/* {line} */);
    //    var width = __src.col,
	//		height = __src.row,
	//		size = __size,// || 3,
     ...
     *	图片降噪，中值滤波
     *	<b>Method</b>
     *	把小于指定面积的噪声点抹去(Mat src, int size, int borderType, Mat dst )
     *	<b>Parameters</b>
        ...
        if (__src.type && __src.type == "CV_GRAY") {
            ...
            for (i = height - 1; i>1; i--) {
                ...
                for (j = width - 1; j > 1; j--) {
                    ...
                    if (pixData[0] === 255) {
                        ////// 8邻域的右下部分如果保含0像素则不用判断，因为其面积必然大于阈值
                        // 8邻域中的九个点，如果白点少于4个则置为黑点
                        if (__src.at("uchar", j + 1, i + 1)[0] + __src.at("uchar", j, i + 1)[0] + __src.at("uchar", j + 1, i)[0] + __src.at("uchar", j - 1, i - 1)[0] + __src.at("uchar", j, i - 1)[0] + __src.at("uchar", j - 1, i)[0] + __src.at("uchar", j - 1, i + 1)[0] + __src.at("uchar", j + 1, i - 1)[0] >= 765)
